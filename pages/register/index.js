...
function Login() {
  ...
  const handleOTPSubmit = async () => {
    ...
    let compare = await fetch("/api/otp/compare", {
      method: "POST",
      body: JSON.stringify({
        typed: typedOTP.toString(),
        hash: sentOTP.toString(),
      }),
    });

    compare = await compare.json();
  ...
  return (
    <>
      ...
      {isOtpSent && (
        <div className="fixed inset-0 h-full w-full z-20 bg-black/80 flex justify-center pt-32">
          <div className="p-7 bg-white w-[500px] h-fit rounded">
            <h1 className="text-2xl font-bold">Enter the OTP</h1>
            <p className="text-sm mt-2">We have sent an otp to {phone}</p>

            <input
              type="text"
              className="h-12 border px-6 rounded w-full tracking-widest mt-6"
              placeholder="XXXXXX"
              value={typedOTP}
            ...
            <div className="flex items-center justify-end mt-6">
              <button
                ...
                className="text-neutral-800 hover:underline mr-5"
              >
                Cancel
              </button>
              <button
                onClick={() => handleOTPSubmit()}
                className="h-12 px-8  bg-[#023E8A] text-white rounded"
              >
                Submit
              </button>
            </div>
